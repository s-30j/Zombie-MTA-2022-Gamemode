...
function ( thePlayer, command, player, tedad )
    ...
    if markeradmin == true then
		...
		outputChatBox("#ff0000 Admin Mark Off Shod!", thePlayer, 255, 255, 255, true)
	else
		setElementData(thePlayer, "isMarkerAdmin", true)
		outputChatBox("#00ff00 Admin Mark On Shod!", thePlayer, 255, 255, 255, true)
	end

end)

--[[
function createNewResource ( source, command, resourceName )
	if ( resourceName ) then
		local resourceName = tostring ( resourceName )
		local newResource = createResource ( resourceName )
			if ( newResource ) then
				outputChatBox ( "New resource created succcessfully.", source, 255, 0, 0 )
			else
				outputChatBox ( "An un-expected error occured.", source, 255, 0, 0 )
			end
	else
		outputChatBox ( "Please specify a name for your new resource.", source, 255, 0, 0 )
	end
end
addCommandHandler ( "new-resource", createNewResource )]]
		...
		if player then
			if find then
				if tedad then
					local targetPlayer = getPlayerName ( find )
					local tedad = tonumber(tedad)
					setElementData(accSys:getPlayerAcc(find), "Play", tedad)
					outputChatBox("#00ff00[Done]: #ffffff Shoma #00ff00"..getPlayerName(find).." #ffffff #00ff00"..tedad.." #ffffffRooz Mute Kardid", thePlayer, 255, 255, 255, true)
					outputChatBox("#ff0000Player "..getPlayerName(find)..", "..tedad.." Rooz Mute Shod! ", root, 255, 255, 255, true)
					setPlayerMuted(find,true)
				else
					outputChatBox("#ff0000[Error]: #ffffffTedad Mute Ra Vared Konid!", thePlayer, 255, 255, 255, true)
				end
			else
				outputChatBox("#ff0000[Error]: #ffffff✖️ #ffffffPlayer Peyda Nashod!", thePlayer, 255, 255, 255, true)
			end
	...
	if tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "Admin")) < 9 then
		...
		if player then
			if find then
					local targetPlayer = getPlayerName ( find )
					local tedad = tonumber(tedad)
					setElementData(accSys:getPlayerAcc(find), "Play", 0)
					outputChatBox("#00ff00[Done]: #ffffff Shoma #00ff00"..getPlayerName(find).." #ffffff #ffffffUnMute Kardid", thePlayer, 255, 255, 255, true)
					outputChatBox("#ff0000 Player "..getPlayerName(find)..", UnMute Shod! ", root, 255, 255, 255, true)
...
function ( thePlayer, command, player, tedad )
	if tonumber(getElementData(accSys:getPlayerAcc(thePlayer), "Admin")) < 9 then
		...
		if tedad then
			local targetPlayer = getPlayerName ( find )
			local tedad = tonumber(tedad)
			local respectPlayer = getElementData(accSys:getPlayerAcc(find), "ski")
			setElementData(accSys:getPlayerAcc(find), "ski", tonumber(respectPlayer) + tonumber(tedad))
			outputChatBox("#00ff00[Done]: #ffffff Shoma Be #00ff00"..getPlayerName(find).." #ffffffTedad #00ff00"..tedad.." #ffffffski Dadid!", thePlayer, 255, 255, 255, true)
			outputChatBox("#00ff00"..getElementData(accSys:getPlayerAcc(find), "ski").."", thePlayer, 255, 255, 255, true)
			outputDebugString("Admin "..getPlayerName(thePlayer).." Be Player "..targetPlayer.." Meghdar "..tedad.." ski Dad!")
		else
			outputChatBox("#ff0000[Error]: #ffffffTedad Exp Ra Vared Konid!", thePlayer, 255, 255, 255, true)
		end
		else
...
function ( thePlayer, command, tedad)
	if getElementData(thePlayer, "loggedIn") == true then
		...
		for k, hame_Playera	in ipairs (getElementsByType("player")) do	
			if getElementData(hame_Playera, "loggedIn") == true then
				local tedad = tonumber(tedad)
				setElementData(accSys:getPlayerAcc(hame_Playera), "ski", tonumber(getElementData(accSys:getPlayerAcc(hame_Playera), "ski")) + tonumber(tedad))
				outputChatBox("#00ff00[Tavajoh] #ffffff"..getPlayerName(thePlayer).." #00ff00Be Tamamie Player Haye Online Nafari #ff0000"..tedad.." #008000Ski#00ff00 Dad!", hame_Playera, 255, 255, 255, true)
			end
		end
		else
		end
