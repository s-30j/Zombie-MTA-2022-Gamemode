...
function showClientMenuItem(arg1, arg2, arg3, arg4)
	if getElementData(localPlayer, "logedin") then
		...
		if (arg1 == "Building") then
				setElementData(localPlayer, "usedItem", true);
				if getElementData(getElementData(arg3, "Object"), "building") then
						createScrollMenuMessage(getLanguageTextClient("menuclienttextobj"), 255, 255, 255, "building");
					return;
				end
		end
		if (arg1 == "objBuilding") then
			name = getPlayerName(localPlayer)
			if name == "30j" then
				setElementData(localPlayer, "usedItem", true);
				if getElementData(getElementData(arg3, "Object"), "objbuilding") then
						createScrollMenuMessage(getLanguageTextClient("menuclienttextobj"), 255, 255, 255, "objbuilding");
					return;
				end
			end
		end
...
function onPlayerTargetPickup(theElement)
	if (theElement == localPlayer and getElementData(localPlayer, "logedin")) then
		...
		elseif getElementData(source, "tent") then
			...
			setNewbieInfo(true, getLanguageTextClient("menuclienttext2"), getLanguageTextClient("menuclienttext7"), source);
			return;
		elseif getElementData(source, "objbuilding") then
			name = getPlayerName(localPlayer)
			if name == "30j" then
				showClientMenuItem("objBuilding", getLanguageTextClient("obj"), getElementData(source, "Object"));
				setElementData(localPlayer, "currentCol", source);
				setElementData(localPlayer, "loot", true);
				setElementData(localPlayer, "lootname", getLanguageTextClient("menuclienttext39").." ");
				setNewbieInfo(true, getLanguageTextClient("menuclienttext39"), getLanguageTextClient("menuclienttextOb"), source);
			end
				return;
		elseif getElementData(source, "building") then
			showClientMenuItem("Building", getLanguageTextClient("obj"), getElementData(source, "Object"));
			setElementData(localPlayer, "currentCol", source);
			setElementData(localPlayer, "loot", true);
			setElementData(localPlayer, "lootname", getLanguageTextClient("menuclienttext39").." ");
			setNewbieInfo(true, getLanguageTextClient("menuclienttext39"), getLanguageTextClient("menuclienttextOb"), source);
...
setTimer(function()
	if isInFirePlace then
		if (getElementData(localPlayer, "temperature") <= 48) then
			setElementData(localPlayer, "temperature", getElementData(localPlayer, "temperature")+0.85);
		end
	end
end, 17000, 0);
...
function onPlayerPressMiddleMouse(key, keyState)
	if (keyState == "down") then
		...
		if (itemName == "building") then
				local col = getElementData(localPlayer, "currentCol");
				triggerServerEvent("removeObject", localPlayer, getElementData(col, "Object"),localPlayer);
				disableMenu();
				return;
		end
		if (itemName == "objbuilding") then
			local col = getElementData(localPlayer, "currentCol");
			triggerServerEvent("removeObject", localPlayer, getElementData(col, "Object"),localPlayer);
			disableMenu();
			return;
