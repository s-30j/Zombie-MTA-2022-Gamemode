...
function addPlayerStats(player, data, value)
	...
	elseif (data == "temperature") then
		...
		elseif (current + value <= 20) then
			setElementData(player, data, 20);
		else
			setElementData(player, data, (current + value));
		end
...
if getElementData(source, "logedin") then
	...
	if (mType == 0) then
		...
		for _,v in ipairs(nearbyPlayers) do
			...
			exports["[ZB]Local"]:CreateServerLog("team", ""..messagePrefix.."  "..string.gsub((getPlayerName(source).." :  "..message), '#%x%x%x%x%x%x', ''))
		end
	...
	outputChatBox( "#ff0000 Shoma Login Nakardid", source, 255, 255, 255, true)
end
end);
...
if getElementData(player, "logedin") then
	...
	outputChatBox("#ff0000 Shoma Login Nakardid", player, 255, 255, 255, true);
end
end);
