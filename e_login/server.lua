...
addEventHandler("onPlayerJoin", getRootElement(), function()
	...
	setElementData(source,"buildcreate", true)
	triggerClientEvent("Login=BugFix",getRootElement(),source)
end)
...
function()
	for k, player in ipairs (getElementsByType("player")) do	
		if getElementData(player, "loggedIn") == true then
			triggerEvent("Login=BugFix",getRootElement(),player)
...
function PlayerSerial(client)
		...
		if (account == false) then
			...
			triggerEvent("Login=BugFix",getRootElement(),client)
		...
		triggerClientEvent("Login=BugFix",getRootElement(),client)
		dbQuery(onLoginStageTwo, {username, client}, exports.mysql:getMySQLC(), "SELECT * FROM `user` WHERE `UserName` = ? LIMIT 1", username)
...
function onLoginStageTwo(qh, username, client)
	...
	if tonumber(getElementData(meAccounto, "Play")) == 0 then
	else	
