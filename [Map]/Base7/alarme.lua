...
RestricLocation["location1"] = { -718.8076171875 ,972.9384765625 ,12.132812 } -- Local 1
RestricLocation["location2"] = {  -664.923828125 ,936.6025390625 ,43.132812  } -- Local 2
TeleportLocation = {   -715.529296875 ,913.7392578125 ,12.317836761475   } -- Local de TP
--GroupName = "Map18" -- Nome da Gang ou Grupo ACL
...
function EnterPlace ( theElement )
	if getElementData(theElement, "logedin") then
		if getElementData(theElement, "loggedIn") then
			...
			if tonumber(getElementData(accSys:getPlayerAcc(theElement), "Map")) == 20 then
				return
				end
...
function sendMsgOwners( PlayerID )
	...
	for k, acwarn in ipairs (getElementsByType("player")) do
		if getElementData(acwarn , "loggedIn") == true then
			...
			if tonumber(getElementData(accSys:getPlayerAcc(acwarn), "Map")) == 20 then
